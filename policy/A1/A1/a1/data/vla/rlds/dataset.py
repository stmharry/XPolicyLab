...
# from a1.vla.constants import ACTION_PROPRIO_NORMALIZATION_TYPE
from a1.data.vla.rlds import obs_transforms, traj_transforms
from a1.data.vla.rlds.utils import goal_relabeling, task_augmentation
from a1.data.vla.rlds.utils.data_utils import (
    allocate_threads,
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    # print("************** olmo/data/vla/rlds/dataset.py, make_dataset_from_rlds, after get_dataset_statistics ")
    ...
    # print("************** olmo/data/vla/rlds/dataset.py, make_dataset_from_rlds, before dl.DLataset.from_rlds ")
    try:
        dataset = dl.DLataset.from_rlds(builder, split=split, shuffle=shuffle, num_parallel_reads=num_parallel_reads)
    except Exception as e:
        print(f"dl.DLataset.from_rlds error: {e}")
    # print("************** olmo/data/vla/rlds/dataset.py, make_dataset_from_rlds, after dl.DLataset.from_rlds ")
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    # Filter out trajectories that are too short to provide the requested number of future actions
    # Ensures: effective_traj_len = traj_len - future_action_window_size >= 1
    ...
    dataset = dataset.filter(lambda traj: tf.shape(traj["action"])[0] > future_action_window_size)
    dataset = dataset.traj_map(
        partial(
...
def make_interleaved_dataset(
    ...
    *,
    ...
    # print("************ make_interleaved_dataset: before make_dataset_from_rlds ")
    # Get Dataset Sizes
    ...
    # print("************ make_interleaved_dataset: after make_dataset_from_rlds ",)
    ...
    log.info("Applying frame transforms on dataset...")
    dataset = apply_frame_transforms(dataset, **frame_transform_kwargs, train=train)
