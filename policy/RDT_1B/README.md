...
`RDT_1B` is the XPolicyLab/RoboDojo adapter for the corresponding policy. It keeps integration-facing scripts at this directory level and leaves the original or vendored implementation in the nested source tree when present.
...
| `rdt/` | Vendored upstream code, policy-specific assets, or helper scripts. |
...
| `text_encoder_path` | Runtime or checkpoint option consumed by this adapter. |
| `vision_encoder_path` | Runtime or checkpoint option consumed by this adapter. |
| `model_root` | Runtime or checkpoint option consumed by this adapter. |
...
| Variable | Notes |
|---|---|
| `CFLAGS` | Optional override used by the local scripts or upstream runtime. |
| `DATA_DIR` | Optional override used by the local scripts or upstream runtime. |
| `DATA_TAG` | Optional override used by the local scripts or upstream runtime. |
| `EMBED_DIR` | Optional override used by the local scripts or upstream runtime. |
...
| `LANG_EMBED_DIR` | Optional override used by the local scripts or upstream runtime. |
| `LDFLAGS` | Optional override used by the local scripts or upstream runtime. |
| `NCCL_DEBUG` | Optional override used by the local scripts or upstream runtime. |
| `NCCL_IB_DISABLE` | Optional override used by the local scripts or upstream runtime. |
| `NCCL_IB_HCA` | Optional override used by the local scripts or upstream runtime. |
