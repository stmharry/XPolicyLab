...
`ACT` is the XPolicyLab/RoboDojo adapter for the corresponding policy. It keeps integration-facing scripts at this directory level and leaves the original or vendored implementation in the nested source tree when present.
...
| `conda_env.yaml` | Supplemental documentation or environment metadata. |
| `detr/` | Vendored upstream code, policy-specific assets, or helper scripts. |
...
# Example: install dependencies for the ACT policy adapter.
bash install.sh
...
| `kl_weight` | Runtime or checkpoint option consumed by this adapter. |
| `chunk_size` | Runtime or checkpoint option consumed by this adapter. |
| `hidden_dim` | Runtime or checkpoint option consumed by this adapter. |
| `dim_feedforward` | Runtime or checkpoint option consumed by this adapter. |
| `temporal_agg` | Runtime or checkpoint option consumed by this adapter. |
| `device` | Runtime or checkpoint option consumed by this adapter. |
| `ckpt_dir` | Runtime or checkpoint option consumed by this adapter. |
| `policy_class` | Runtime or checkpoint option consumed by this adapter. |
| `num_epochs` | Runtime or checkpoint option consumed by this adapter. |
| `position_embedding` | Runtime or checkpoint option consumed by this adapter. |
| `lr_backbone` | Runtime or checkpoint option consumed by this adapter. |

Frequently used environment variables detected in the adapter scripts:

| Variable | Notes |
|---|---|
| `ACT_ACTION_DIM` | Optional override used by the local scripts or upstream runtime. |
